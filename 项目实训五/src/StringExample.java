public class StringExample{
    public static void main(String args[]) {
        ...
        if(0<s4.compareToIgnoreCase(s5)) {              //按字典序s4大于s5的表达式。
            System.out.println("按字典序s4大于s5");
        }
        else{
            System.out.println(" 字典序s4小于s5");
        }
        int position=0;
        String path="c:\\java\\jsp\\A.java";
        position=path.lastIndexOf("\\") ;    //获取path中最后出现目录分隔符号的位置
        System.out.println("c:\\java\\jsp\\A.java中最后出现\\的位置:"+position);
        String fileName=path.toString();//获取path中“A.java”子字符串。
        ...
        String s8=(String.valueOf((int)m));
        position=s8.indexOf(".");//String调用valuOf(int n)方法将m转化为字符串对象position=s8.indexOf(".");
